...
_e["table"] = function () {
    ...
    tblConf["funcs"]["init"] = function(container_id,
        ...
        this["tplMain"] = '<table class="{{tblClass}}">'+
            ...
            ' <select id="selColname"><option id="selRCol" value="-1">所有</option>'+
            ...
            '</td><tr>'+
                // 复杂查询特殊处理：不需要的可以隐藏掉
            '<tr id="trdept"><td colspan={{colCount}}>部门 <select id="cqdept">' +
    ...
    function tblInit(tbl){
        ...
        var load_default_cq = function(){
            ...
            for(var i1=1;i1<units.length;i1++){
                units[i1].checked=true
            }
            ...
            tbl["cq_unit"]=cq_unit
            tbl["cq_time"]=[_e["dateToInt"](tody),_e["dateToInt"](_e["addDate"](tody,1))]
        }
        //初始默认的 复杂查询
        load_default_cq()
    ...
    tblConf["funcs"]["loadData"] = function(qc){
        ...
        if(qc && (qc["qseq"] || qc["oseq"])){
            ...
            if(tbl["ext_condi"]){
                fd.append("qseq",tbl["ext_condi"]["qseq"]) // query column seq
                fd.append("qverb",tbl["ext_condi"]["qverb"]) //
                fd.append("qpt",tbl["ext_condi"]["qpt"]) // query pattern
            }
        }
