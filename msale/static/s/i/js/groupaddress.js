 ...
 */
 ...
 function groupAddress(){
   window.location.href="groupconfirm.html?comid="+groupComId+"&aId="+groupId+"&n="+number
 }
function newAddress(){
...
function loadAddress(addressJson) {
    ...
    for (var i = 0; i < addressJson.address.length; i++) {
        var url="location.href='groupAddresseDit.html?addressid="+addressJson.address[i].id+'&id='+addressJson.address[i].userid+'&comid='+groupComId+'&aId='+groupId+'&n='+number
        url+="'"
        var divFlex = document.createElement("div")
        divFlex.setAttribute("class", "flex")
        divFlex.innerHTML = '<ul  onclick="conversion(this);" class="hover hover'+addressJson.address[i].flag+'"><i id="change" class="icon iconfont icon-circleo" flag="'+addressJson.address[i].flag+'" ></i></ul>' +
