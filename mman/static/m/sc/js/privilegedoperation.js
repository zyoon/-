...
function load_old() {
    if (window.location.search.length > 10) {
        ...
        document.querySelector("#orderForm").value = outmemo
        var xhrx2 = new XMLHttpRequest()
        var url = "/sc/expressstock/getitem?id=" + _e["stripscript"](stranfer_id)
        ...
        xhrx2.onreadystatechange = function () {
            if (xhrx2.readyState == 4 && xhrx2.status == 200) {
                ...
                var selectTbody1 = document.querySelector("#commodityList")
                for (var i = 0; i < items.iteminfo.length; i++) {
                    var tr=document.createElement("tr")
                    tr.setAttribute("data-id",items.iteminfo[i][10])
                    tr.innerHTML =
                        '<td>' + items.iteminfo[i][1] + '</td>'+
                        '<td data-id="'+items.iteminfo[i][8]+'">' + items.iteminfo[i][11] + '</td>'+
                        '<td>' + items.iteminfo[i][2] + '</td>'+
                        '<td> <input class="amount" type="number" value="" style="width: 80px;" storageCapacity="'+items.iteminfo[i][3] +'"/>' + items.iteminfo[i][6] + '</td>'
                    ...
                    tr.querySelector(".amount").value=items.iteminfo[i][4]
                }
            }
        }
    }
      ...
      coldefs = [{
        ...
        row_actions = [{cls: "doerow", func: function (tbl, rows) {
                ...
                for(var i = 0;i< tr_id.length;i++){
                    if(datas[12] == tr_id[i].getAttribute("data-id")){
                        if(datas[0] == Number(tr_id[i].children[0].innerHTML)){
        ...
        document.querySelector("#planCommodity").addEventListener("click", function () {
          ...
          var dlg = _e.dialog({id:"dlgData",width:"500px",
              title:"确认出库计划商品",
              mainBody:" <div><i>出库人 : </i><h3 id='handlerin'></h3>" +
              "<i>出库部门 : </i><h3 id='deptin'></h3>" +
              "</div>" +
              ...
              actions:[{id:"btn",title:"确定出库",func:function(){
                if(strabsferout_appened){
                  _e["msgBox"]({  msg: "已经点击过出库，切勿重复入库"})
                  return
                }
                strabsferout_appened=true
                  xhr.open("POST","/sc/expressstock/inerttransferout"+_e["jurisdiction"](),true)
                  xhr.send(fd)
                  xhr.onreadystatechange = function(){
                      if (xhr.readyState==4 && xhr.status==200){
            ...
            if (stranfer_id) {
              ...
              fd.append("handlerout",staffinfo.staff[0])
              fd.append("deptout",cqdept.value)
              fd.append("outmemo",document.querySelector("#orderForm").value)
        ...
        t.funcs.init.call(t, "tbl",
            coldefs, row_actions, rows_actions, "table", "__THE__TABLE__", ext_row, "/sc/expressstock/getstocks"+_e["jurisdiction"](), after_Load)
...
if(drafts){
    _e["gethtml"](document.querySelector('#commodityList'),'privilegedoperation',['amount'])
}else{
    var t2 = window.setInterval("_e.sethtml(document.querySelector('#commodityList'),'privilegedoperation',['amount'])",10000);
}
