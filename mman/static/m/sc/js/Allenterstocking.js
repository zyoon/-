        ...
        func: function(tbl, rows) { //把入库商品添加到入库列表中
            ...
            loadDate({
                ...
                callback: function(d) {
                    ...
                    var titleName = "入库编辑"
                    if (tbl.data[rows][1] == "5" || tbl.data[rows][1] == "1" || tbl.data[rows][1] == "3" || tbl.data[rows][1] == "4") {
                        titleName = "关闭"
                    }
...
function addComList(e) { //通过计划的id取得其它采购的id
    ...
    tr.innerHTML = '<td class="commodityRow" comNum="' + addCom.children[0].getAttribute("comNum") + '" comId="' + addCom.children[0].getAttribute("comId") + '" style="width: 80px;">' + addCom.children[0].innerHTML + '</td>' +
        ...
        '<td style="width: 100px;"><input style="width: 100px;" class="supplier" supplierId="" onkeyup="supplier(this)"/><ul style="width:140px;background-color: #ffffff;margin: 0;padding: 0;position: fixed;overflow-y: auto;"></ul></td>' +
...
document.querySelector("#planCommodity").addEventListener("click", function() { //入库提交
    ...
    for (var i = 0; i < orderCom.length; i++) {
        ...
        fd.append("stockingprice", Number(orderCom[i].querySelector(".price").value) * 100)
